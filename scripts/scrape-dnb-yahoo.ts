#!/usr/bin/env tsx
/**
 * Scrape DNB Historical Data from Yahoo Finance HTML
 *
 * Since Yahoo Finance API is restricted, we'll fetch the HTML page
 * and parse the data table directly
 */
import { Pool } from "pg";
...
async function scrapeYahooData(): Promise<YahooBar[]> {
  ...
  console.log('Fetching HTML from Yahoo Finance...');
  ...
  // Find all table rows in the historical data table
  $('table tbody tr').each((_, row) => {
    ...
    if (cells.length >= 7) {
      ...
      if (!isNaN(open) && !isNaN(high) && !isNaN(low) && !isNaN(close) && !isNaN(adjClose) && !isNaN(volume)) {
        bars.push({ date, open, high, low, close, adjClose, volume });
      }
    }
  });
...
async function main() {
  ...
  console.log("SCRAPE DNB HISTORICAL DATA FROM YAHOO FINANCE");
  console.log("=".repeat(70));
  console.log("\nNote: This scrapes visible data from the Yahoo Finance page\n");
  ...
  try {
    ...
    const result = await scrapeYahooData();

    console.log(`✓ Scraped ${result.length} bars from HTML table`);

    if (result.length === 0) {
      console.log("✗ No data found in HTML. Table structure may have changed.");
      return;
    }
