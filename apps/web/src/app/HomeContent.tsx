...
export default function HomeContent({ stats }: { stats: SystemStats }) {
  ...
  const securities = useCountUp(stats.securities, 1500);
  const dataPoints = useCountUp(stats.data_points, 2000);
  ...
  return (
    <>
      ...
      <main style={{ minHeight: "100vh", position: "relative", zIndex: 1, color: "#e6edf3", padding: "32px 16px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>

          {/* Hero */}
          <header style={{ marginBottom: 48, borderBottom: "1px solid #21262d", paddingBottom: 32 }}>
            <div style={{
              fontSize: 11, fontFamily: "monospace", color: "#8b949e", marginBottom: 16,
            ...
            <h1 style={{
              fontSize: 28, fontWeight: 700, marginBottom: 16, color: "#e6edf3",
            ...
            <p style={{
              fontSize: 15, color: "#8b949e", lineHeight: 1.7, maxWidth: 720,
          ...
          <div style={{
            ...
            <StatBox label="Securities Covered" value={securities} suffix="" />
            <StatBox label="OHLCV Data Points" value={dataPoints.toLocaleString()} suffix="pts" />
          ...
          <section ref={modulesReveal.ref} style={{ marginBottom: 48 }}>
            <SectionHeading visible={modulesReveal.visible}>Active Stock Research</SectionHeading>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
              <FeatureCard href="/stocks" title="Stock Screener & Analytics"
                ...
                tags={[{ label: "Charts", color: "#3b82f6" }, { label: "Excel Models", color: "#10b981" }, { label: "ML", color: "#06b6d4" }]}
                visible={modulesReveal.visible} delay={0} />
              ...
              <FeatureCard href="/news" title="Intelligence Terminal"
                ...
                visible={modulesReveal.visible} delay={2} />
            </div>
          </section>
          ...
          <section style={{ marginBottom: 48 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
              <FeatureCard href="/volatility/obx" title="OBX Volatility Dashboard"
                ...
                visible={modulesReveal.visible} delay={3} />
              <FeatureCard href="/correlation" title="Correlation Matrix"
                ...
                tags={[{ label: "Heatmap", color: "#06b6d4" }, { label: "Rolling", color: "#3b82f6" }]}
                visible={modulesReveal.visible} delay={4} />
              <FeatureCard href="/options" title="Options Analytics"
                ...
                tags={[{ label: "Greeks", color: "#10b981" }, { label: "P&L Builder", color: "#3b82f6" }, { label: "IV Skew", color: "#f59e0b" }]}
                visible={modulesReveal.visible} delay={5} />
              <FeatureCard href="/portfolio" title="Portfolio Optimizer"
          ...
          <section ref={capabilitiesReveal.ref} style={{ marginBottom: 56 }}>
            <SectionHeading visible={capabilitiesReveal.visible}>Analytics Engine</SectionHeading>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
              {[
                ...
                <CapabilityCard key={cap.title} title={cap.title} accent={cap.accent} visible={capabilitiesReveal.visible} delay={i}>
                  {cap.text}
                </CapabilityCard>
              ))}
            </div>
          ...
          <section ref={upcomingReveal.ref} style={{ marginBottom: 56 }}>
            <SectionHeading visible={upcomingReveal.visible}>Upcoming</SectionHeading>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
              {[
                ...
                <CapabilityCard key={cap.title} title={cap.title} accent={cap.accent} visible={upcomingReveal.visible} delay={i}>
                  {cap.text}
                </CapabilityCard>
              ))}
            </div>
          ...
          {/* Footer */}
          <footer style={{ paddingTop: 28, borderTop: "1px solid #21262d", fontSize: 11.5, color: "#8b949e", fontFamily: "system-ui, -apple-system, sans-serif" }}>
...
function SectionHeading({ children, visible }: { children: React.ReactNode; visible: boolean }) {
  return (
    <h2 style={{
      fontWeight: 600, marginBottom: 24, color: "#8b949e",
...
function StatBox({ label, value, suffix, live }: { label: string; value: string | number; suffix: string; live?: boolean }) {
  return (
    <div style={{ background: "#161b22", border: "1px solid #21262d", borderRadius: 6, padding: "18px 20px" }}>
      <div style={{
        fontSize: 10, color: "#8b949e", textTransform: "uppercase", letterSpacing: "0.08em",
        marginBottom: 10, fontWeight: 600, fontFamily: "system-ui, -apple-system, sans-serif",
        display: "flex", alignItems: "center", gap: 6,
      }}>
        {label}
        {live && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#10b981", display: "inline-block", animation: "pulse 2s ease-in-out infinite" }} />}
      </div>
      <div style={{
        ...
        {value}
        {suffix && <span style={{ fontSize: 12, color: "#8b949e", fontWeight: 500 }}>{suffix}</span>}
      </div>
    </div>
  );
}
...
function CapabilityCard({ title, accent, children, visible, delay }: {
  title: string; accent: string; children: React.ReactNode; visible: boolean; delay: number;
}) {
  return (
    <div className="cap-card" style={{
      ...
      <h4 style={{ fontSize: 12, fontWeight: 600, marginBottom: 10, color: "#e6edf3", fontFamily: "system-ui, -apple-system, sans-serif", letterSpacing: "0.03em" }}>
        {title}
      </h4>
...
const COLOR_MAP: Record<string, string> = {
  "#3b82f6": "#3b82f6", "#10b981": "#10b981", "#f59e0b": "#f59e0b",
  "#ef4444": "#ef4444", "#06b6d4": "#06b6d4",
};
...
function TagPill({ label, color }: { label: string; color: string }) {
  ...
  return (
    <span style={{
      ...
      height: 18, padding: "0 7px", borderRadius: 10, fontSize: 9,
      fontFamily: "system-ui, -apple-system, sans-serif", fontWeight: 600,
      letterSpacing: "0.03em", textTransform: "uppercase" as const, whiteSpace: "nowrap" as const,
      background: `${hex}18`, color: `${hex}cc`,
    }}>
      {label}
    </span>
  );
