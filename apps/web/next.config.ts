...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      // Security headers — all routes (improves trust score with ISP/SafeSurf filters)
      {
        source: '/:path*',
        headers: [
          ...
          { key: 'Content-Security-Policy', value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' https://*.supabase.co https://*.vercel.app; frame-ancestors 'none'" },
        ],
      },
